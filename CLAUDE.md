...
- **`redis_client.py`** — Two responsibilities:
  1. **Pub-Sub subscriber**: Loops on `pc_inspect` channel with exponential backoff reconnect (5s→10s→30s→60s→120s). On command, verifies HMAC signature (registry secret), filters by `target`/`targets` (hostname) and optionally `target_uuid`/`targets_uuid` (system UUID, AND condition — disambiguates PCs that share a hostname), then dispatches.
  2. **Heartbeat sender**: Connects to `/ws/client` via WSS with `truststore` SSL context, sends 60s heartbeat `{"type":"heartbeat","hostname":"...","ip_address":"...","uuid":"...","version":"..."}`. Same exponential backoff reconnect.
